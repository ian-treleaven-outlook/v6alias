//! SHA-256 (FIPS 180-4), used only for deterministic state comparison, not authentication.
//! Kept dependency-free so the local milestone builds from the existing offline cache.
...
mod tests {
    ...
    #[test]
    fn sha256_standard_known_answer_vectors() {
        for (input, expected) in [
            ...
            assert_eq!(sha256(input.as_bytes()), expected);
        }
        assert_eq!(
